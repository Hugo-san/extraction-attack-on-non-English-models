...
def print_best(metric, samples, name1, scores1, name2=None, scores2=None, n=20,totoal_samples=None):
    ...
    for i, idx in enumerate(tqdm(idxs, desc='Processing samples')):
        ...
        occ = check_partial_match(samples[idx],mask = 20)
        ...
        for infor in personal_information:
            if len(personal_information[infor]) > 0:
                for line in personal_information[infor]:
                    occ_infor += check_partial_match(line,mask = 0)
                    personal_information_all += " " +line
...
def check_partial_match(output_text,mask):
    ...
    reader = pysubstringsearch.Reader(index_file_path='ja.idx',)
...
def main():
    ...
    csv_filename = f'{args.text_len}_XL_Lower_best_samples_scores_Ja.csv'
    df_XL_Lower.to_csv(csv_filename, index=False, escapechar='\\')
    
    df_XL_Lower_overall = pd.DataFrame(XL_Lower_result_overall)
    csv_filename = f'{args.text_len}_XL_Lower_best_samples_scores_overall_Ja.csv'
