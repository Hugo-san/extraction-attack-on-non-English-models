...
def has_repeated_chars(window_text, threshold=0.5):
...
def store_all_samples_to_csv(metric,all_samples,name1,scores1,name2=None,scores2=None,N=None):
    ...
    for i, idx in enumerate(tqdm(idxs, desc='Storing samples')):
        ...
        occ = -1
        personal_information = extract_important_info_de(all_samples[idx])
...
def print_best(metric, samples, name1, scores1, name2=None, scores2=None, n=20,totoal_samples=None, window = False):
    """
    ...
    for i, idx in enumerate(tqdm(idxs, desc='Processing samples')):
        ...
        parts = samples[idx].split('\n', maxsplit=1)
        occ_1 = 0
        occ_2 = 0
        first_part = parts[0]
        second_part = parts[1] if len(parts) > 1 else ""
        if occ == 0 and window == False:
        # check the index file separately
        ...
        personal_information = extract_important_info_de(samples[idx])
        ...
        for infor in personal_information:
            if len(personal_information[infor]) > 0:
                for line in personal_information[infor]:
                    occ_infor += check_partial_match(line)
                    personal_information_all += " " +line
...
def extract_important_info_de(text):
    ...
    r'(?!\d)',                        
    text)
    filtered_phones = [phone for phone in phones if sum(len(part) for part in phone if part.isdigit()) >= 8]
    
    keywords = ["twitter", "instagram", "wechat", "snapchat"]
    pattern = r'(?i)\b(?:' + '|'.join(keywords) + r')\s+([a-zA-Z_][a-zA-Z0-9_]{5,19})\b'
...
def main():
    ...
    samples = []
    window_samples = []
    scores = {"XL": [], "S": [], "Sliding_window": [], "zlib": [], "Lower": []}
    ...
    with tqdm(total=args.N) as pbar:
        for i in range(num_batches):
            ...
            while len(input_ids) < args.batch_size:
                    ...
                    prompt = "".join(cc_data[r1][r2:r2+100])
    ...
    csv_filename = f'{args.text_len}_XL_best_samples_scores_German.csv'
    df_XL.to_csv(csv_filename, index=False, encoding='utf-8', escapechar='\\')
    
    df_XL_overall = pd.DataFrame(XL_result_overall)
    csv_filename = f'{args.text_len}_XL_best_samples_scores_overall_German.csv'
    df_XL_overall.to_csv(csv_filename, index=False,encoding='utf-8', escapechar='\\')
    ...
    csv_filename = f'{args.text_len}_S_XL_best_samples_scores_German.csv'
    df_S_XL.to_csv(csv_filename, index=False,encoding='utf-8', escapechar='\\')

    df_S_XL_overall = pd.DataFrame(S_XL_result_overall)
    csv_filename = f'{args.text_len}_S_XL_best_samples_scores_overall_German.csv'
    df_S_XL_overall.to_csv(csv_filename, index=False,encoding='utf-8', escapechar='\\')
    ...
    XL_Lower_result, XL_Lower_result_overall = print_best(metric, generated_samples_clean, "PPL-XL", scores["XL"], "PPL-XL-Lower", scores["Lower"],n=args.best_n,totoal_samples=args.N)
    
    df_XL_Lower = pd.DataFrame(XL_Lower_result)
    csv_filename = f'{args.text_len}_XL_Lower_best_samples_scores_German.csv'
    df_XL_Lower.to_csv(csv_filename, index=False, escapechar='\\')
    
    df_XL_Lower_overall = pd.DataFrame(XL_Lower_result_overall)
    csv_filename = f'{args.text_len}_XL_Lower_best_samples_scores_overall_German.csv'
    df_XL_Lower_overall.to_csv(csv_filename, index=False,encoding='utf-8', escapechar='\\')
    ...
    window_result, window_result_overall = print_best(metric, window_generated_samples_clean, "PPL-XL", scores["XL"], "PPL-XL-Sliding-window", scores["Sliding_window"],args.best_n,args.N,window = True)
    
    df_window = pd.DataFrame(window_result)
    csv_filename = f'{args.text_len}_window_best_samples_scores_German.csv'
    df_window.to_csv(csv_filename, index=False,encoding='utf-8', escapechar='\\')
    
    df_window_overall = pd.DataFrame(window_result_overall)
    csv_filename = f'{args.text_len}_window_best_samples_scores_overall_German.csv'
    df_window_overall.to_csv(csv_filename, index=False,encoding='utf-8', escapechar='\\')
    ...
    zlib_result, zlib_result_overall = print_best(metric, generated_samples_clean, "PPL-XL", scores["XL"], "Zlib", scores["zlib"],args.best_n,args.N,window = False)
    
    df_zlib = pd.DataFrame(zlib_result)
    csv_filename = f'{args.text_len}_zlib_best_samples_scores_German.csv'
    df_zlib.to_csv(csv_filename, index=False,encoding='utf-8', escapechar='\\')

    df_zlib_overall = pd.DataFrame(zlib_result_overall)
    csv_filename = f'{args.text_len}_zlib_best_samples_scores_overall_German.csv'
    df_zlib_overall.to_csv(csv_filename, index=False,encoding='utf-8', escapechar='\\')
